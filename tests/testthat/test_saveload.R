...
test_that("save-load-save", {
  ...
  model <- uwotlite::umap(X, n_neighbors = 4, ret_model = TRUE)
  model_file <- tempfile(tmpdir = tempdir())
  model <- uwotlite::save_uwot(model, file = model_file)
  model2 <- uwotlite::load_uwot(file = model_file)
  new_file <- tempfile(tmpdir = tempdir())
  uwotlite::save_uwot(model2, file = new_file)
  expect_true(file.exists(new_file))

  modelm <- uwotlite::umap(X, n_neighbors = 4, metric = list("euclidean" = 1:5, "euclidean" = 6:10), ret_model = TRUE)
  modelm_file <- tempfile(tmpdir = tempdir())
  modelm <- uwotlite::save_uwot(modelm, file = modelm_file)
  modelm2 <- uwotlite::load_uwot(file = modelm_file)
  new_filem <- tempfile(tmpdir = tempdir())
  uwotlite::save_uwot(modelm2, file = new_filem)
