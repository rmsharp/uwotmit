...
test_that("save-load-save", {
  ...
  model <- uwotmit::umap(X, n_neighbors = 4, ret_model = TRUE)
  model_file <- tempfile(tmpdir = tempdir())
  model <- uwotmit::save_uwot(model, file = model_file)
  model2 <- uwotmit::load_uwot(file = model_file)
  new_file <- tempfile(tmpdir = tempdir())
  uwotmit::save_uwot(model2, file = new_file)
  expect_true(file.exists(new_file))

  modelm <- uwotmit::umap(X, n_neighbors = 4, metric = list("euclidean" = 1:5, "euclidean" = 6:10), ret_model = TRUE)
  modelm_file <- tempfile(tmpdir = tempdir())
  modelm <- uwotmit::save_uwot(modelm, file = modelm_file)
  modelm2 <- uwotmit::load_uwot(file = modelm_file)
  new_filem <- tempfile(tmpdir = tempdir())
  uwotmit::save_uwot(modelm2, file = new_filem)
...
test_that("save-load relative path", {
  ...
  # Use withr::with_dir to temporarily change directory
