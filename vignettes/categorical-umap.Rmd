---
title: "Dimensionality Reduction for Encoded Categorical Data with uwotlite"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Dimensionality Reduction for Encoded Categorical Data with uwotlite}
...
This vignette demonstrates how to use `uwotlite` for dimensionality reduction,
complementing the categorical encoding approaches from
[Chapter 17 of Tidy Modeling with R](https://www.tmwr.org/categorical).

While Chapter 17 focuses on encoding categorical variables, `uwotlite` provides
UMAP (Uniform Manifold Approximation and Projection) for reducing high-dimensional
encoded data to visualizable or more manageable dimensions.
...
After encoding categorical variables (as shown in the embedmit categorical
encoding vignette), you may have many numeric features. UMAP can reduce these
to 2 dimensions for visualization or to a smaller number of dimensions for
modeling.
...
```{r umap-viz-price, fig.cap="UMAP projection colored by Sale Price"}
...
```{r umap-viz-neighborhood, fig.cap="UMAP projection colored by Neighborhood"}
...
```{r param-comparison, fig.height=8, fig.cap="Effect of n_neighbors on UMAP embedding"}
set.seed(123)
...
```{r supervised-umap, fig.cap="Supervised UMAP uses Sale Price to guide the projection"}
set.seed(123)
...
- [embedmit categorical encoding vignette](https://rmsharp.github.io/embedmit/)
