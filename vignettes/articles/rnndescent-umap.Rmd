...
* Run `uwotmit::umap` with `nn_method` set to the `graph` item in the result of
...
* Run `uwotmit::umap_transform` with `nn_method` set to the result of
  `rnnd_query`.
  
If you don't want to transform new data, then it's even easier:

* Run `rnndescent::rnnd_knn` on your training data.
* Run `uwotmit::umap` with `nn_method` set to the result of `rnnd_knn`.
