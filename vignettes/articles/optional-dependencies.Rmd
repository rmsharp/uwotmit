...
in `uwotmit::umap` as long as your `metric` is either `"euclidean"`, `"cosine"` or
`"correlation"`. This should be a bit faster than the default of Annoy in most
cases. If you use `uwotmit::umap2` then you will get HNSW by default without having
...
"nndescent"` in `uwotmit::umap`. `rnndescent` can handle many metrics, so see its
[documentation](https://jlmelville.github.io/rnndescent/articles/metrics.html)
for more information. If you use `uwotmit::umap2` and do not load `RcppHNSW`, then
you will use this method by default without having to specify `nn_method`.
You can also use sparse matrices as input to `uwotmit::umap2`. See the
